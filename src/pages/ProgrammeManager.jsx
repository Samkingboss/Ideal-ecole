...
export default function ProgrammeManager({ user, selectedClasse, supabase, onUpdate }) {
  ...
  const loadObjectifs = async (matiere) => {
    setSelectedMatiere(matiere); setView("objectifs")
    const { data } = await supabase.from("objectifs").select("*").eq("matiere_id", matiere.id).order("nom")
    setObjectifs(data || [])
  }
  ...
  const handleSave = async () => {
    ...
    if (view === "matieres") {
      ...
      if (editItem) { await supabase.from("objectifs").update({ nom: formVal }).eq("id", editItem.id) }
      else { await supabase.from("objectifs").insert({ nom: formVal, matiere_id: selectedMatiere.id }) }
      const { data } = await supabase.from("objectifs").select("*").eq("matiere_id", selectedMatiere.id).order("nom")
      setObjectifs(data || [])
    }
  ...
  const currentItems = view === "matieres" ? matieres : objectifs
  const currentTable = view === "matieres" ? "matieres" : "objectifs"
  const label = view === "matieres" ? "Matiere" : "Objectif"
  const emoji = view === "matieres" ? "📚" : "🎯"
  ...
  return (
    <div style={{padding:"1rem 1.2rem 3rem"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:"1rem",flexWrap:"wrap"}}>
        <span onClick={()=>{setView("matieres");setSelectedMatiere(null);}} style={{fontSize:12,fontWeight:700,color:view==="matieres"?"var(--accent)":"var(--muted)",cursor:"pointer"}}>Matieres</span>
        {selectedMatiere && (<><span style={{color:"var(--muted)"}}>›</span><span style={{fontSize:12,fontWeight:700,color:view==="objectifs"?"var(--accent)":"var(--muted)",cursor:"default"}}>{selectedMatiere.nom}</span></>)}
      </div>
      ...
      <div style={{background:"var(--card)",borderRadius:14,border:"1px solid var(--border)",overflow:"hidden",marginBottom:"1rem"}}>
        <div style={{background:"#0d2a3b",color:"#fff",padding:"8px 14px",fontSize:11,fontWeight:700,textTransform:"uppercase",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span>{view === "matieres" ? "Matieres" : "Objectifs — "+selectedMatiere?.nom}</span>
        ...
        {currentItems.length === 0 ? (
          ...
          <div key={item.id} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",borderBottom:"1px solid var(--border)"}}>
            ...
            <div style={{flex:1,fontSize:13,fontWeight:600,cursor:view==="matieres"?"pointer":"default"}} onClick={()=>{ if(view==="matieres") loadObjectifs(item); }}>
              {item.nom}{view==="matieres"&&<span style={{fontSize:10,color:"var(--accent)",marginLeft:6}}>Voir →</span>}
            </div>
      ...
      <button className="btn btn-primary" onClick={()=>{setShowForm(true);setEditItem(null);setFormVal("");}} style={{width:"100%"}}>
        + Ajouter {view==="matieres"?"une matiere":"un objectif"}
      </button>
    </div>
  )
}
