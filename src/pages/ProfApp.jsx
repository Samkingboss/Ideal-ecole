...
export default function ProfApp({ user, onLogout }) {
  ...
  const [newDevoir, setNewDevoir] = useState({ matiere: '', description: '', date_rendu: '' })
  // Discipline states
  ...
  const saveDevoir = async (e) => {
    e.preventDefault()
    if (!selectedClasse) return alert('Sélectionnez une classe')
    setLoading(true)
    ...
    if (!error) {
      setNewDevoir({ matiere: '', description: '', date_rendu: '' })
      loadData()
    }
    setLoading(false)
  }

  const deleteDevoir = async (id) => {
  ...
  const getProgressionData = () => {
    ...
    const classCps = checkpoints.filter(cp => {
      const matchNew = cp.classe_id === selectedClasse?.id
      const matchOld = planifications.find(pl => pl.id === cp.planification_id && pl.classe_id === selectedClasse?.id)
