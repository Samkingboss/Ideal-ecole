    ...
    {vue === 'bulletin' && <div className="bm-report-wrap">
      ...
      <article className="bm-report">
        <section className="bm-report-page">
          ...
          <div className="bm-performance-summary"><b>{texteScore(scoreGlobal)}</b><p>{resumePerformance(scoreGlobal)}</p><small>{domainesEvalues.length}/{tousDomaines.length} domaines évalués sur la période · Indicateur visuel : TB 100 % · B 75 % · M 50 % · AR 25 %. Sans classement ni effet automatique sur le passage.</small></div>
          </section>
          ...
          <section className="bm-report-section"><div className="bm-section-heading"><span>02</span><div><h2>Mon chemin de progrès</h2><p>Un indicateur d’observation par domaine, sans note académique ni comparaison entre les enfants.</p></div></div><div className="bm-progress-path">{tousDomaines.map(d => <div key={d.id}><span>{d.titre}</span><i><b style={{ width: `${d.score}%` }} /></i><strong>{d.score ? `${d.score}%` : '—'}</strong></div>)}</div></section>
