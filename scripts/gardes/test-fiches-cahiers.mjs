...
test('F9 · une fiche occupe seule sa page',()=>assert.ok(paginerFiches(fiches).every(p=>p.length===1)))
test('F10 · pagination déterministe : N élèves donnent N pages',()=>assert.deepEqual(paginerFiches(fiches).map(p=>p.map(f=>f.id)),[['m'],['a'],['k']]))
test('F10b · 1, 2 et 10 élèves donnent exactement 1, 2 et 10 pages',()=>{
  for (const n of [1, 2, 10]) assert.equal(paginerFiches(Array.from({length:n},(_,i)=>({id:String(i)}))).length,n)
})
...
test('F18 · impression protège les fiches et force une page A4 par enfant',()=>{assert.match(interfaceSource,/page-break-inside:avoid/);assert.match(interfaceSource,/paginerFiches\(fiches, 1\)/);assert.doesNotMatch(interfaceSource,/grid-template-rows:1fr 1fr/)})
test('F18b · logo officiel visible',()=>assert.match(interfaceSource,/<img src="\/logo-ideal\.png" alt="Logo IDEAL"/))
